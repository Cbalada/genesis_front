...
export default function Page() {
 ...
 const upsertProperty = (list: Property[], next: Property) => {
 ...
 return <main className="min-h-screen bg-background text-foreground">
  ...
  {host&&<HostModal onClose={()=>setHost(false)} user={user} token={token} onAuth={()=>setAuth(true)} onCreated={(p)=>{setProperties(list=>upsertProperty(list,p)); setNotice('¡Tu alojamiento fue publicado correctamente!')}} onUpdated={(p)=>setProperties(list=>upsertProperty(list,p))}/>} 
          ...
          {imagesList.length > 1 && (
            <>
              <button
                type="button"
                onClick={prevImage}
                className="absolute left-3 top-1/2 -translate-y-1/2 flex size-9 items-center justify-center rounded-full bg-card/80 text-foreground shadow-md hover:bg-card transition-transform hover:scale-105"
                aria-label="Anterior"
              >
                <ChevronLeft size={20} />
              </button>
              ...
              <button
                type="button"
                onClick={nextImage}
                className="absolute right-3 top-1/2 -translate-y-1/2 flex size-9 items-center justify-center rounded-full bg-card/80 text-foreground shadow-md hover:bg-card transition-transform hover:scale-105"
        ...
        <div className="flex flex-col gap-5 p-6">
          ...
          <p className="leading-7 text-muted-foreground">
            {property.description ||
              'Un espacio pensado para descansar, descubrir el destino y vivir una estadía memorable con todo lo necesario.'}
          </p>
          ...
          <div className="flex items-center justify-between">
            <span>
              <b className="text-xl">${money(property.pricePerNight)}</b> / noche
            </span>
            <button
              disabled={busy}
              onClick={reserve}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-foreground/40 p-5">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-3xl bg-card flex flex-col">
        <div className="flex items-center justify-between border-b border-border px-6 py-5">
          <div>
            <p className="text-sm font-semibold text-primary">Para anfitriones</p>
            <h2 className="mt-1 text-2xl font-semibold">
              {createdProperty ? 'Imágenes del alojamiento' : 'Publicá tu espacio'}
            </h2>
          </div>
          <button
            type="button"
        ...
        {!user ? (
          ...
          <div className="max-h-[calc(90vh-88px)] overflow-y-auto px-6 py-6 space-y-6">
            <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-sm text-emerald-800">
              <strong className="font-semibold block mb-1">¡Propiedad creada con éxito!</strong>
              Se ha registrado &quot;{createdProperty.title}&quot;. Ahora podés subir las fotos oficiales a Google Drive.
            </div>
          ...
          <div className="max-h-[calc(90vh-88px)] overflow-y-auto px-6 py-6">
            <div className="flex flex-col gap-5">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (nextValue === '' || /^\d+$/.test(nextValue)) {
                          updateField('bathrooms', nextValue)
                        } else {
                          setErrors((current) => ({
                            ...current,
                            bathrooms: 'Solo se aceptan enteros. Ingresá nuevamente.',
                          }))
                        }
                      }}
                      placeholder="Ej: 2"
                  ...
                  <label className="block text-sm font-medium">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const nextValue = e.target.value
                        if (nextValue === '' || /^\d+$/.test(nextValue)) {
                          updateField('bedrooms', nextValue)
                        } else {
                          setErrors((current) => ({
                            ...current,
                            bedrooms: 'Solo se aceptan enteros. Ingresá nuevamente.',
                          }))
                        }
                      }}
                      placeholder="Ej: 3"
              ...
              >
                {busy ? 'Guardando datos...' : 'Siguiente: Subir fotos'}
              </button>
            </div>
          </div>
        )}
